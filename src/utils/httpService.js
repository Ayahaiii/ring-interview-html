...
//let host = `192.168.0.157:8069`

let host = `192.168.0.132:8069` //卜滴
let baseUrl = `http://${host}/ringinterviewapi`;
// let baseAuthUrl = `http://i.ringdata.com/uums`;
  ...
  return instance.post(url, data).then((result) => {
    ...
    if(result && result.status === 200) {
      if (res != null && res.access_token != null && res.access_token != '') {
        // this.$cookies.set("tokenWeb", res.access_token, 24 * 60 * 60, '/', 'ringdata.com', false);
        // this.$cookies.set("userInfo", JSON.stringify(res.userInfo), 24 * 60 * 60, '/', 'ringdata.com', false);
