<template>
  ...
  >
    ...
    <el-table-column
      ...
      min-width="100"
      align="center"
      :prop="selfProps.managerName"
      label="负责人"
      >
      <template slot-scope="scope">
        <span>{{scope.row.managerName == null ? '--' : scope.row.managerName}}</span>
      </template>
    </el-table-column>
    <el-table-column
      fixed
      ...
      min-width="100"
      label="状态"
      :prop="selfProps.status"
    >
      <template slot-scope="scope">
        <span v-if="scope.row.status == 0" class="init-btn">初始化</span>
        <span v-if="scope.row.status == 1" class="dispath-btn">已分配</span>
        <span v-if="scope.row.status == 2" class="pending-btn">进行中</span>
        <span v-if="scope.row.status == 3" class="success-btn">已完成</span>
      </template>
    </el-table-column>
    <slot></slot>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <div v-if="useIngTime" :class="ingDayGanttType(scope.row, month.full_date, 'months')"></div>
            <div v-if="useDoneTime" :class="doneDayGanttType(scope.row, month.full_date, 'months')"></div>
          </template>
        </el-table-column>
      </el-table-column>
    </template>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <div v-if="useIngTime" :class="ingDayGanttType(scope.row,t.full_date, 'week')"></div>
            <div v-if="useDoneTime" :class="doneDayGanttType(scope.row,t.full_date, 'week')"></div>
          </template>
        </el-table-column>
      </el-table-column>
    </template>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <div v-if="useIngTime" :class="ingDayGanttType(scope.row, t.full_date)"></div>
            <div v-if="useDoneTime" :class="doneDayGanttType(scope.row, t.full_date)"></div>
          </template>
        </el-table-column>
      </el-table-column>
    </template>
...
export default {
  ...
  computed: {
    ...
    selfProps() {
      return {
        ...
        code: "code", //样本编号
        name: "name", // 姓名
        managerName: "managerName", //负责人
        status: "status",
        id: "id", // id字段
        pid: "pid", // pid字段
        startDate: "startDate", // 初始化开始
        endDate: "endDate", // 初始化结束
        realStartDate: "realStartDate", // 已分派开始
        realEndDate: "realEndDate", // 已分派结束
        ingStartDate: "ingStartDate", //进行中开始时间
        ingEndDate: "ingEndDate", // 进行中结束时间
        doneStartDate: "doneStartDate", //已完成开始时间
        doneEndDate: "doneEndDate", //已完成结束时间
  ...
  methods: {
    ...
    generationDays(year, month, isLeap = false, week = false) {
      let start = Number(this.startDate.substring(8, 10))
      let end = Number(this.endDate.substring(8, 10))
      ...
      //   }
      // } else {
        for (let i = start-1; i < dates_num; i++) {
          days.push({
            date: i,
            name: `${i}日`,
            id: uuidv4(),
    ...
    ingDayGanttType(row, date, unit = "days") {
      let start_date = row[this.selfProps.ingStartDate];
      let end_date = row[this.selfProps.ingEndDate];
      let between = dayjs(date).isBetween(start_date, end_date, unit);
      if (between) {
        return "wl-ing-on";
      }
      ...
      let end = dayjs(end_date).isSame(date, unit);
      if (start && end) {
        return "wl-ing-on wl-ing-full";
      }
      if (start) {
        return "wl-ing-on wl-ing-start";
      }
      if (end) {
        return "wl-ing-on wl-ing-end";
      }
    },
    doneDayGanttType (row, date, unit = "days") {
      let start_date = row[this.selfProps.doneStartDate];
      let end_date = row[this.selfProps.doneEndDate];
      let between = dayjs(date).isBetween(start_date, end_date, unit);
      if (between) {
        return "wl-done-on";
      }
      ...
      let end = dayjs(end_date).isSame(date, unit);
      if (start && end) {
        return "wl-done-on wl-done-full";
      }
      if (start) {
        return "wl-done-on wl-done-start";
      }
      if (end) {
        return "wl-done-on wl-done-end";
      }
    },
    // 以下是时间计算类函数 ------------------------------------------------------时间计算---------------------------------------
    /**
...
.wl-gantt {
  ...
  .wl-real-on {
    ...
    margin-top: -$gantt_item_half;
    height: $gantt_item;
    background: #1db0cf; //rgba(250, 167, 146, .6);
    transition: all 0.4s;
  }
  .wl-real-start {
    left: 50%;
  ...
  .wl-ing-on {
    ...
    margin-top: -$gantt_item_half;
    height: $gantt_item;
    background: #ea913a; //rgba(250, 167, 146, .6);
    transition: all 0.4s;
  }
  .wl-ing-start {
    left: 50%;
  ...
  .wl-done-on {
    ...
    margin-top: -$gantt_item_half;
    height: $gantt_item;
    background: #77b936; //rgba(250, 167, 146, .6);
    transition: all 0.4s;
  }
  .wl-done-start {
    left: 50%;
